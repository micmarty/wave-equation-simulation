...
% Slide 33/34 modified with example data (from instruction)
...
% Visualize equlibrium (linear function, like yE = cx + d)
% 1) Find c (using given condition): dy/dx(x=0, t) = 0.8, hence c = 0.8
% 2) Find d (using given condition): y(x=L, t) = -0.28, hence 
%   -0.28 = 0.8 * L + d => d = -0.28 - 0.8 = -1.08
plot(x,0.8*x - 1.08,'g');

% Add second plot for comparison
hold on; 
plot(x,f(:,N),'b');
